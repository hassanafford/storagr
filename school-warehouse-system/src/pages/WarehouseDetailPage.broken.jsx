...
import { getItemsByWarehouse, createItem, updateItem, deleteItem, updateItemQuantity, createTransaction } from '../services/itemService';
...
function WarehouseDetailPage() {
  ...
  const loadWarehouseData = async () => {
    try {
      ...
      const itemsData = await getItemsByWarehouse(id);
      setItems(itemsData);
      
      // Load transactions for this warehouse
      const transactionsData = await getTransactionsByWarehouse(id);
          ...
          <div className="md:hidden w-full">
            {items.length === 0 ? (
              ...
              <div className="space-y-4 w-full">
                {items.map((item) => (
                  <div key={item.id} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm w-full">
        ...
        <div className="max-w-3xl mx-auto w-full overflow-x-hidden">
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6">
            <div className="flex">
              ...
              <p className="text-blue-700 text-sm">اختر العنصر المراد إرجاعه وحدد الكمية والظروف</p>
            </div>
          </div>
          ...
          <form onSubmit={handleReturnSubmit} className="space-y-4 w-full">
            ...
            <div>
              ...
              <input
                type="number"
                min="1"
                max={items.find(item => item.id == returnData.item_id)?.quantity || 1}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">الظروف</label>
              <select
        ...
        <div className="max-w-3xl mx-auto w-full overflow-x-hidden">
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6">
            <div className="flex">
              ...
              <p className="text-blue-700 text-sm">اختر العناصر المراد تبديله وحدد الكميات والمستلم</p>
            </div>
          </div>
          
          <form onSubmit={handleExchangeSubmit} className="space-y-4 w-full">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">العنصر المرسل</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">الكمية المرسلة</label>
              <input
                type="number"
                min="1"
                max={items.find(item => item.id == exchangeData.outgoing_item_id)?.quantity || 1}
                value={exchangeData.outgoing_quantity}
                onChange={(e) => setExchangeData({...exchangeData, outgoing_quantity: parseInt(e.target.value)})}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">العنصر المستلم</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">الكمية المستلمة</label>
              <input
                type="number"
                min="1"
                value={exchangeData.incoming_quantity}
                onChange={(e) => setExchangeData({...exchangeData, incoming_quantity: parseInt(e.target.value)})}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
      ...
      {activeTab === 'log' && (
        <div className="w-full overflow-x-auto">
          ...
          {transactions.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              <p>لا يوجد حركات حالية لهذا المخزن</p>
            </div>
          )}
        </div>
      )}
