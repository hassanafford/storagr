  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const activities = transactionsData.slice(0, 5).map(transaction => {
        ...
        const itemName = transaction.item_name || 'عنصر محذوف';
        const quantity = Math.abs(transaction.quantity || 0);
        const recipient = transaction.recipient || 'غير محدد';
        const warehouseName = transaction.warehouse_name || 'مخزن غير محدد';
        const userName = transaction.user_name || 'مستخدم غير معروف';

        return {
          id: transaction.id,
          type: type,
          title: title,
          description: `${quantity} ${itemName} - ${recipient}`,
          ...
          {recentActivities.length === 0 ? (
            ...
            <div className="space-y-4">
              {recentActivities.map((activity) => (
                <div 
                  key={activity.id} 
                  className="flex items-start border-b border-gray-200 pb-3 last:border-0 last:pb-0 cursor-pointer hover:bg-gray-50 p-2 rounded"
                  onClick={() => navigate('/transactions')}
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {activity.warehouse} | {activity.user} | {activity.time}
                    </p>
                  </div>
                </div>
              ))}
